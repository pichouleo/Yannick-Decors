...
import { BrushDivider, BrushAccent, BrushUnderline } from '@/components/BrushStroke'
...
function Hero() {
  return (
    <section
      ...
      aria-label="Présentation Yannick Décors"
    >
      {/* Texture peinture fond */}
      <PaintTextureBg />
      ...
      <div className="container-site relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-end pb-20 pt-8">
          {/* Gauche — texte */}
          <div>
            <span className="section-tag text-terra/90">Artisan peintre indépendant · Normandie</span>

            <h1 className="font-title text-cream mb-6" style={{ fontSize: 'clamp(36px, 5vw, 58px)', lineHeight: '1.1' }}>
              Votre habitat,
              <br />
              <span className="text-terra">peint avec soin.</span>
            </h1>
            ...
            <p className="text-cream/70 text-lg leading-relaxed mb-8 max-w-lg">
              ...
              <strong className="text-cream">70&nbsp;km autour du Neubourg</strong> — Eure et Seine-Maritime.
              Devis gratuit sous 48h.
            </p>

            {/* Téléphone bien visible */}
            <div className="bg-terra/20 border-l-4 border-terra px-6 py-4 mb-8">
              <p className="text-cream/70 text-sm font-body font-semibold uppercase tracking-wider mb-1">Appelez directement</p>
              <a
                href={PHONE_HREF}
                className="font-title text-cream hover:text-terra transition-colors"
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Link href="/realisations" className="btn-outline border-cream text-cream hover:bg-cream hover:text-slate text-lg">
                Voir les réalisations
              </Link>
            </div>
          </div>

          {/* Droite — palette décorative */}
      ...
      {/* Transition brush vers section suivante */}
      ...
      <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="grid" width="60" height="60" patternUnits="userSpaceOnUse">
            <path d="M 60 0 L 0 0 0 60" fill="none" stroke="#F5F4F0" strokeWidth="0.5"/>
          </pattern>
        </defs>
...
function HeroVisual() {
  return (
    <div className="relative flex items-end justify-center h-full min-h-[400px]">
      <Image
        src="/yannick-decors-hero.png"
        alt=""
        width={600}
        height={400}
        className="object-contain opacity-20 w-full h-auto"
    ...
    <section className="bg-cream py-24" id="services">
      <div className="container-site">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-0 border-2 border-border">
          {items.map((item, i) => (
            <Link
              key={item.title}
              href="/services"
              className={`group flex flex-col gap-5 p-8 hover:bg-slate hover:text-cream transition-colors duration-200 ${
                i < 3 ? 'border-r-0 sm:border-r-2 border-b-2 sm:border-b-0 lg:border-b-0 border-border last:border-0' : ''
              } border-b-2 lg:border-b-0 border-r-0 lg:border-r-2 last:border-r-0`}
              style={{ borderRight: i < 3 ? '2px solid #D6D4CE' : 'none', borderBottom: '0' }}
            >
        ...
        <div className="text-center mt-10">
          <Link href="/services" className="btn-slate text-lg">
...
function Stats() {
  const stats = [
    { value: '15+', label: "Années d'expérience",    sub: 'depuis 2010' },
    { value: '200+', label: 'Chantiers réalisés',    sub: 'en Normandie' },
    { value: '70km', label: "Rayon d'intervention",  sub: 'autour du Neubourg' },
    { value: '48h', label: 'Délai devis gratuit',     sub: 'réponse rapide' },
  ]
  return (
      ...
      <div className="container-site">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-0 border-2 border-white/10">
          {stats.map((s, i) => (
            <div
              key={s.label}
              className={`px-8 py-10 text-center ${i < 3 ? 'border-r-0 md:border-r-2 border-white/10' : ''} border-b-2 lg:border-b-0 border-white/10 last:border-b-0 last:border-r-0`}
              style={{ borderRight: i < 3 ? '2px solid rgba(255,255,255,0.1)' : 'none' }}
            >
              <p className="font-title text-terra" style={{ fontSize: '56px', lineHeight: '1' }}>{s.value}</p>
  ...
  const reasons = [
    {
      ...
      desc: "C'est moi qui réalise vos travaux, du début à la fin. Pas de sous-traitance. Vous avez affaire directement à votre peintre — disponible, présent et responsable.",
    },
    {
      num: '02',
      title: 'Devis gratuit & transparent',
      desc: 'Chaque devis est détaillé : fournitures, temps de travail, nombre de couches. Pas de mauvaises surprises. Réponse sous 48h après visite.',
    },
    {
      num: '03',
      title: 'Qualité & garantie',
      desc: "J'utilise exclusivement des peintures professionnelles haut de gamme. Mon travail est soigné et garanti. Votre satisfaction est ma priorité.",
    },
  ]
  return (
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          <div>
            ...
            <p className="section-lead mb-10">
              En Normandie, les artisans sérieux sont rares. Je vous propose un travail consciencieux, des conseils honnêtes et une relation directe sans intermédiaire.
            </p>
            <a href={PHONE_HREF} className="btn-phone">
          ...
          <div className="space-y-0 border-2 border-border">
            {reasons.map((r, i) => (
              <div
                key={r.num}
                className={`flex gap-6 p-8 ${i < reasons.length - 1 ? 'border-b-2 border-border' : ''}`}
              >
                <span className="font-title text-terra/30" style={{ fontSize: '48px', lineHeight: '1', flexShrink: 0 }}>
                  {r.num}
                </span>
                <div>
    ...
    <section className="bg-slate py-24">
      ...
      <div className="container-site">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12">
          ...
          <Link href="/realisations" className="btn-outline border-cream text-cream hover:bg-cream hover:text-slate whitespace-nowrap self-start sm:self-auto">
            Voir tout
          </Link>
        </div>
        ...
        <div className="grid sm:grid-cols-3 gap-0 border-2 border-white/10">
          {items.map((item, i) => (
            <Link
              key={item.src}
              href="/realisations"
              className={`group relative aspect-[4/3] block overflow-hidden ${i < 2 ? 'border-r-2 border-white/10' : ''}`}
            >
              <Image
                src={item.src}
                alt={item.alt}
...
function Testimonials() {
  const temoignages = [
    {
      text: "Yannick a réalisé la peinture de notre salon et notre chambre. Travail impeccable, ponctuel et très professionnel. Les finitions sont parfaites. Je recommande sans hésiter !",
      author: 'Marie-Claire D.',
      lieu: 'Le Neubourg',
    },
    {
      text: "Excellent artisan, sérieux et de confiance. Il a rénové l'ensemble de notre façade avec un résultat remarquable. Le devis était clair et respecté. Très satisfait.",
      ...
      <div className="container-site">
        ...
        <div className="grid md:grid-cols-2 gap-0 border-2 border-border">
          {temoignages.map((t, i) => (
            <div
              key={t.author}
              className={`p-10 ${i === 0 ? 'border-b-2 md:border-b-0 md:border-r-2 border-border' : ''}`}
            >
              ...
              <div className="flex items-center gap-4 pt-4 border-t-2 border-border">
                ...
                <div className="ml-auto flex gap-0.5">
                  {[...Array(5)].map((_, j) => (
                    <StarIcon key={j} className="w-4 h-4 text-terra" />
                  ))}
                </div>
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="section-lead mb-6">
              Basé à <strong className="text-dark">Saint Opportune du Bosc</strong>, j'interviens dans un rayon de{' '}
              <strong className="text-dark">70 km autour du Neubourg</strong>, principalement dans l'Eure et la Seine-Maritime.
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              {villes.map((v) => (
                <span key={v} className="border-2 border-border text-dark text-base px-3 py-1.5 font-body">
                  {v}
                </span>
              ))}
          ...
          {/* Carte SVG simple */}
          <div className="border-2 border-border bg-cream p-8">
            <ZoneMapSvg />
          </div>
        </div>
      </div>
...
function ZoneMapSvg() {
  return (
    <svg viewBox="0 0 400 360" className="w-full max-w-sm mx-auto" role="img" aria-label="Carte zone d'intervention — 70km autour du Neubourg">
      ...
      {/* points villes */}
      {[[260,150],[155,125],[305,215],[145,235],[240,270],[130,175]].map(([cx,cy],i) => (
        <circle key={i} cx={cx} cy={cy} r="5" fill="#3D4451"/>
      ))}
  ...
  return (
    <section className="bg-slate py-24">
      <div className="container-site text-center">
        ...
        <h2 className="font-title text-cream mb-6" style={{ fontSize: 'clamp(40px, 6vw, 72px)' }}>
          Prêt à démarrer votre projet ?
        </h2>
        <p className="text-cream/60 text-xl max-w-xl mx-auto mb-10 leading-relaxed">
          Contactez-moi pour un devis gratuit et sans engagement. Je vous réponds sous 48h.
        </p>
